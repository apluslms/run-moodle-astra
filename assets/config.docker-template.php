...
// $_SERVER['HTTP_HOST'] is a workaround: support both localhost and
// moodle hostnames while testing in local containers.
// The web browser in the host machine uses the "localhost" name while
// the grader container connects to Moodle with the Docker internal hostname
// "moodle".
// Officially, Moodle supports only one hostname and this workaround could
// cause other issues.
// The host machine could use "moodle" hostname instead of "localhost" if
// the "moodle" hostname is mapped to localhost (127.0.0.1) in the host system
// (/etc/hosts in Linux)
// (the host is the underlying machine that runs containers).
if (isset($_SERVER['HTTP_HOST'])) {
    $host = $_SERVER['HTTP_HOST'];
} else if (!empty(getenv('MOODLE_DOCKER_WEB_HOST'))) {
...
} else if (!empty(getenv('MOODLE_DOCKER_WEB_PORT'))) {
    $port = getenv('MOODLE_DOCKER_WEB_PORT');
}
if (!empty($port) && strpos($host, ':') === false) { // no port number in $host
    $CFG->wwwroot .= ":{$port}";
}

// default blocks for new courses
$CFG->defaultblocks_override = 'activity_modules,astra_setup';

$CFG->dataroot  = '/var/www/moodledata';
$CFG->admin     = 'admin';
$CFG->directorypermissions = 0777;
$CFG->smtphosts = 'mailhog:1025';
// Moodle does not accept an email address like noreply@localhost (only top-level domain)
$CFG->noreplyaddress = 'noreply@domain.local';
